...
const DebatePreview = ({ debate }) => {
    ...
    return (
        <section className='debate-preview'>
            ...
            <NavLink to={`/debate/${nextDebateId}`} className='skip-btn btn'>דלג</NavLink>
            {nextDebateId}
            <a href="/" className='comments-btn'><img src={commentsIcon} alt="" /></a>

            <div>
                <div className='comments-title'>תגובות</div>

                <ul className='clean-list'>
                    {debate.comments.map(comment => < li key={comment._id} >
                        {comment.text}
                        By: User
                    </li>)}
                </ul>
